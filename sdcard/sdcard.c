...
)
{
	...
	} while (d != 0xFF && _micros() < t + wt);	/* Wait for card goes ready or timeout */
...
)
{
	...
	} while (token == 0xFF && _micros() < t + timeout);
...
{
	BYTE n, cmd, ty, ocr[4];
	const uint32_t timeout = 1000000; /* Initialization timeout = 1 sec */
	uint32_t t;
	...
	if (send_cmd(CMD0, 0) == 1) {			/* Put the card SPI/Idle state */
		...
		if (send_cmd(CMD8, 0x1AA) == 1) {	/* SDv2? */
			...
			if (ocr[2] == 0x01 && ocr[3] == 0xAA) {				/* Is the card supports vcc of 2.7-3.6V? */
				while (_micros() < t + timeout && send_cmd(ACMD41, 1UL << 30)) ;	/* Wait for end of initialization with ACMD41(HCS) */
				if (_micros() < t + timeout && send_cmd(CMD58, 0) == 0) {		/* Check CCS bit in the OCR */
			...
			while (_micros() < t + timeout && send_cmd(cmd, 0)) ;		/* Wait for end of initialization */
			if (_micros() >= t + timeout || send_cmd(CMD16, 512) != 0)	/* Set block length: 512 */
				ty = 0;
		}
	}
...
)
{
	...
	spi_send_bytes(b, btx, true, false);
}
