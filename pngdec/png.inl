        ...
        switch (iMarker)
        {
            ...
            //        }
                    ...
                    while (err == Z_OK) {
                        ...
                        #if 0
                        printf("Inflate next_out: %p, next_in: %p, avail_in: %d, state: %p, state_strm: %p, zalloc: %p, zfree: %p\n", d_stream.next_out, d_stream.next_in, d_stream.avail_in, 
                               &d_stream, ((struct inflate_state FAR *)d_stream.state)->strm,
                               d_stream.zalloc, d_stream.zfree);
                        #endif
                    ...
                    if (err == Z_STREAM_END && d_stream.avail_out == 0) {
                        ...
                        iLen = 0; // quit now
                        printf("Stream error: %d: %s, %d\n", err, d_stream.msg, ((struct inflate_state FAR *)d_stream.state)->mode);
                        y = pPage->iHeight;
                        pPage->iError = PNG_DECODE_ERROR;
