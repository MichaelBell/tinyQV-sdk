...
int check_crc;
{
    ...
    for (;;) {
        ...
        case CODELENS:
            ...
            //for (int i = 0; i < 1 << state->lenbits; ++i) printf("%02x %02x %04x\n", state->codes[i].op, state->codes[i].bits, state->codes[i].val);
            ...
            //for (int i = 1 << state->lenbits; i < (1 << state->distbits) + (1 << state->lenbits); ++i) printf("%02x %02x %04x\n", state->codes[i].op, state->codes[i].bits, state->codes[i].val);
        ...
        case DISTEXT:
            ...
            if (state->offset > state->dmax) {
                strm->msg = (char *)"invalid distance too far back 4";
                state->mode = BAD;
                break;
            }
#endif
        ...
        case MATCH:
            ...
            if (state->offset > copy) {         /* copy from window */
                ...
                if (copy > state->whave) {
                    if (state->sane) {
                        strm->msg = (char *)"invalid distance too far back 5";
                        state->mode = BAD;
                        break;
                    }
